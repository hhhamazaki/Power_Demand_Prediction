...
This file contains the CSS styles extracted from `JennyPortal_cyber_4.html`. It can be used as a reference for creating a unified web design prompt.
...
:root {
    ...
    --glow-filter: drop_shadow(0 0 5px var(--accent-color)) drop_shadow(0 0 10px var(--accent-color));
}
...
.mini-link-card {
    ...
    max-width: 100%; /* Ensure mini-link-card does not exceed parent width */
}
...
.mini-link-card a {
    ...
    white-space: normal; /* テキストを改行する */
    overflow: visible; /* コンテンツが隠れないようにする */
