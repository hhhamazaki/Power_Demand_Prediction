...
class AmmoniaPredictionSystem:
    def __init__(self):
        ...
        self.refill_threshold = 80.0
        self.refill_amount = 400.0
    ...
    def detect_refill_timing(self, predicted_ammonia, days_since_refill):
        ...
        if days_since_refill >= 15 and predicted_ammonia <= 150:
            return True
    ...
    def predict(self, data_path, output_path='data/predictions.csv'):
        ...
        for i in range(len(result_df)):
            ...
            if i == 0:
                ...
                # 補給タイミング自動検出
                if i > 0:
                    days_since_refill = sum(result_df.iloc[max(0, i-15):i]['is_refill'] == 0)
                    if self.detect_refill_timing(predicted_ammonia, days_since_refill):
                        ...
                        result_df.iloc[i, result_df.columns.get_loc('is_refill')] = 1
                        print(f"{current_date.strftime('%m/%d')}: 自動補給実行 → {predicted_ammonia:.1f} m^3")
