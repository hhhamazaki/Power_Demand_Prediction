...
複数年の組み合わせでLightGBMモデル学習を実行し、最適な年選択を自動決定する。
        ...
        try:
            ...
            if data_result.returncode != 0:
                return {
                    ...
                    'test_year': test_year,
                    'success': False,
                    'error': f"Data processing failed: {data_result.stderr[:500]}",
            ...
            if model_result.returncode != 0:
                return {
                    ...
                    'test_year': test_year,
                    'success': False,
                    'error': f"Model training failed: {model_result.stderr[:500]}",
                    'execution_time': time.time() - start_time
                }
