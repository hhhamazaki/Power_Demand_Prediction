...
def process_temperature_and_create_dataset(
    ...
    """
    try:
        # 最新日付取得（JSTを明示して日本時間での日付を使用）
        latest_date = (dt.datetime.utcnow() + dt.timedelta(hours=9)).date()
        ...
        os.makedirs(os.path.dirname(Ytest_csv) or '.', exist_ok=True)
