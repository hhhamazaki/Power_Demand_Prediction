...
def calculate_evaluation_metrics(ypred: np.ndarray, ytest: np.ndarray) -> Tuple[float, float]:
    ...
    try:
        ...
        if ypred.shape != ytest.shape:
            ...
            # ytestの行数に合わせてypredを調整（時系列データなので先頭から合わせる）
            min_length = min(ypred.shape[0], ytest.shape[0])
            if ypred.shape[0] > min_length:
                # ypredが大きい場合は先頭部分を使用（時系列の開始位置を合わせる）
                ypred = ypred[:min_length]
                print(f"[OK] ypredを調整: {ypred.shape} (先頭{min_length}行使用)")
            if ytest.shape[0] > min_length:
                # ytestが大きい場合も先頭部分を使用
                ytest = ytest[:min_length]
                print(f"[OK] ytestを調整: {ytest.shape} (先頭{min_length}行使用)")
